    ...
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-slate-800/60">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="flex items-center gap-1.5 rounded-lg bg-cyan-500/10 px-2.5 py-1 text-[11px] font-semibold text-cyan-400 border border-cyan-500/20">
              <Database className="h-3 w-3" />
              SQLite Persistent Architecture
            </span>
            <span className="text-[11px] text-slate-500">WAL Mode Active</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-3 space-y-2">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 rounded-xl px-3.5 py-3 text-left transition-all duration-200 group cursor-pointer ${
                  isActive
                    ? 'bg-gradient-to-r from-cyan-500/15 to-blue-500/10 border border-cyan-500/30 shadow-md'
          ...
          <div className="mt-4 rounded-xl bg-slate-900/40 border border-slate-800/60 p-4 space-y-2.5">
            ...
            <div className="space-y-1 text-[10px] text-slate-400">
              <div className="flex justify-between">
                <span>Database:</span>
                <span className="font-mono text-slate-200">SQLite 3 (WAL)</span>
              </div>
              <div className="flex justify-between">
                <span>Sync Mode:</span>
                <span className="font-mono text-slate-200">NORMAL (Fast ACID)</span>
              </div>
              <div className="flex justify-between">
                <span>Active Facility:</span>
                <span className="font-mono text-cyan-400 truncate max-w-[110px]">{tenant.name}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-9">
          <div className="rounded-2xl bg-slate-900/50 border border-slate-800/60 p-6 shadow-xl backdrop-blur-md min-h-[520px]">
            ...
            {activeSection === 'account' && (
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                {/* Profile Banner */}
                <div className="rounded-xl bg-slate-950/60 border border-slate-800/60 p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="mt-1.5 flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-slate-500">Tenant: {tenant.name}</span>
                      </div>
                    </div>
                  </div>

                  <div className="flex items-center gap-2">
                    <span className="rounded-lg bg-slate-800/80 border border-slate-700/60 px-3 py-1.5 text-[11px] font-mono text-slate-300">
                      ID: {user.user_id}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-xl bg-slate-950/70 border border-slate-800 px-3.5 py-2.5 text-xs font-semibold text-white placeholder-slate-500 outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/30 transition-all"
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Direct Phone / WhatsApp</label>
                    <input
                      type="text"
                      value={phone}
                  ...
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Department / Operational Unit</label>
                    <input
                      type="text"
                      value={department}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  <div className="rounded-xl bg-slate-950/50 border border-slate-800/60 p-4">
                    ...
                    <div className="space-y-3 text-xs">
                      <div className="flex items-center justify-between pb-2 border-b border-slate-800/40">
                        <span className="text-slate-400">Stateless Token</span>
                        <span className="font-mono text-cyan-400 font-semibold">HS256 JWT (Signed)</span>
                      </div>
                      <div className="flex items-center justify-between pb-2 border-b border-slate-800/40">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-slate-300 font-mono">24 Hours (Rolling)</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl bg-slate-950/50 border border-slate-800/60 p-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {user.permissions.map((perm, idx) => (
                        <span key={idx} className="rounded-md bg-slate-800/80 border border-slate-700/60 px-2 py-0.5 text-[10px] font-mono text-cyan-300">
                          {perm}
                        </span>
                      ))}
                    </div>
            ...
            {activeSection === 'facility' && (
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Active Dock Bays</label>
                    <input
                      type="number"
                      value={activeDocks}
                      onChange={(e) => setActiveDocks(Number(e.target.value))}
                      className="w-full rounded-xl bg-slate-950/70 border border-slate-800 px-3.5 py-2.5 text-xs font-semibold text-white outline-none focus:border-cyan-500/50 transition-all"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Target SLA Turnaround (Minutes)</label>
                    <input
                      type="number"
                      value={slaTarget}
                  ...
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Free Dwell Time (Hours)</label>
                    <input
                      type="number"
                      step="0.5"
                      value={freeTime}
                ...
                <div className="pt-2">
                  ...
                  <div className="rounded-xl border border-slate-800 overflow-hidden bg-slate-950/40">
                    <table className="w-full text-left text-xs">
                      <thead className="bg-slate-900/60 border-b border-slate-800 text-[10px] text-slate-400 font-bold uppercase">
                        <tr>
                      ...
                      <tbody className="divide-y divide-slate-800/40">
                        <tr>
                          <td className="px-4 py-2.5 font-mono text-cyan-400">cam-01-gate-inbound</td>
                          <td className="px-4 py-2.5 text-slate-300">ANPR Homography</td>
                          <td className="px-4 py-2.5 text-slate-400">Gate 1 North</td>
                          <td className="px-4 py-2.5"><span className="text-emerald-400 font-bold">ONLINE</span></td>
                        </tr>
                        <tr>
                          <td className="px-4 py-2.5 font-mono text-cyan-400">cam-02-gate-outbound</td>
                          <td className="px-4 py-2.5 text-slate-300">ANPR Homography</td>
                          <td className="px-4 py-2.5 text-slate-400">Gate 2 North</td>
                          <td className="px-4 py-2.5"><span className="text-emerald-400 font-bold">ONLINE</span></td>
                        </tr>
                        <tr>
                          <td className="px-4 py-2.5 font-mono text-cyan-400">cam-03-dock-apron</td>
                          <td className="px-4 py-2.5 text-slate-300">Dock Dwell Cycle</td>
                          <td className="px-4 py-2.5 text-slate-400">Loading Bays 1-6</td>
                          <td className="px-4 py-2.5"><span className="text-emerald-400 font-bold">ONLINE</span></td>
                        </tr>
                        <tr>
                          <td className="px-4 py-2.5 font-mono text-cyan-400">cam-04-roof-leak</td>
                          <td className="px-4 py-2.5 text-slate-300">Specular Concrete Leak AI</td>
                          <td className="px-4 py-2.5 text-slate-400">Godown Bay C-4</td>
                          <td className="px-4 py-2.5"><span className="text-amber-400 font-bold">HAZARD SPIKE</span></td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
            ...
            {activeSection === 'notifications' && (
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <div className="pt-2">
                  <h4 className="text-xs font-bold text-white mb-3">Active Dispatch Channels</h4>
              ...
              <div className="space-y-6 animate-in fade-in duration-200">
                <div className="flex items-center justify-between pb-4 border-b border-slate-800/50">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">Persisted SQLite audit trail of all gate, dock, and security actions</p>
                  </div>

                  <div className="flex items-center gap-2">
                    <button
                      onClick={fetchAuditLogs}
                      className="inline-flex items-center gap-1.5 rounded-lg bg-slate-800 px-3 py-1.5 text-xs font-semibold text-slate-300 hover:bg-slate-700 transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-lg bg-cyan-500/10 border border-cyan-500/30 px-3 py-1.5 text-xs font-semibold text-cyan-400 hover:bg-cyan-500/20 transition-colors"
                    >
                  ...
                  <div className="divide-y divide-slate-800/50 max-h-[380px] overflow-y-auto">
                    {auditLogs.length === 0 ? (
                      ...
                      auditLogs.map((log, idx) => (
                        <div key={idx} className="flex items-center justify-between p-3.5 hover:bg-slate-900/40 transition-colors text-xs">
                          <div className="space-y-0.5">
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-white">{log.action}</span>
                              <span className="font-mono text-[10px] text-cyan-400">ID: {log.id}</span>
                            </div>
                          ...
                          <div className="text-right text-[10px] text-slate-500 font-mono">
                            ...
                            <div className="text-slate-400">{log.user_id || 'System'}</div>
                          </div>
                        </div>
                      ))
                    )}
...
const NotifToggle: React.FC<{ label: string; description: string; defaultOn: boolean }> = ({ label, description, defaultOn }) => {
  ...
  return (
    <div className="flex items-center justify-between rounded-xl bg-slate-950/40 border border-slate-800/50 px-4 py-3.5 hover:border-slate-700/60 transition-colors">
      ...
      <button
        onClick={() => setOn(!on)}
        className={`relative h-6 w-11 rounded-full transition-all duration-300 cursor-pointer ${on ? 'bg-cyan-500 shadow-sm shadow-cyan-500/30' : 'bg-slate-700'}`}
      >
